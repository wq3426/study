...
// 订单状态
OrderStatus.ORDER_RETURNED = -8;//已退货
...
var OrderDetail = {
	...
	initBtnStatus : function() {
		...
		if(this.payStatus == OrderStatus.PAY_CONFIRM){
			$("#pay").linkbutton("disable");
			$("#cancel").linkbutton("disable"); //确认收款后，禁用作废按钮  修改人：DMRain 2015-12-15
		}
		...
		//如果订单状态为已退货(-8)或者为退货(-2)，则禁用作废按钮 
		//修改人：DMRain 2015-12-15
		if(this.orderStatus == OrderStatus.ORDER_RETURNED || this.orderStatus == OrderStatus.ORDER_CANCEL_SHIP){
			$("#cancel").linkbutton("disable"); 
		}
