...
var CheckOut={
	...
	bindAddrEvent:function(){
		...
		$("#saveAddrBtn").click(function(){
			...
			$.ajax({
				url:ctx+'/api/store/storeOrder!changeAddress.do',
	...
	createAddressHtml:function(wrapper){
		...
		var html = "<span class='take_delivery'>"+shipAddr+",(收货人："+shipName+"，手机："+shipMobile+"，电话："+shipTel+"，邮箱："+shipZip+")</span>";
	...
	createOrder:function(btn){
		...
		var options = {
			url : ctx+"/api/store/storeOrder!create.do?ajax=yes",
			type : "POST",
			dataType : 'json',
