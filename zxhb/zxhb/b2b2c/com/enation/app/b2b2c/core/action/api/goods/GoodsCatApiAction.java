...
import com.enation.app.base.MailUtil;
...
@InterceptorRef(value="apiRightStack",params={"apiRightInterceptor.excludeMethods","getStoreGoodsChildJson,edit"})
public class GoodsCatApiAction extends WWAction{
	...
	public String addGoodsCat(){
		...
		try {
			int count1 =  this.storeGoodsCatManager.getStoreCatNameIsExist(store_cat_name,storeMember.getStore_id());
			if(count1 == 0){
	...
	public String sendEmail(){
		...
		if(storeMember != null){
			...
			aa=MailUtil.sendEmail(storeMember.getStore_id(),cat.getName(),newBrand);
			if(aa.equals("0")){
				this.showSuccessJson("邮件发送成功");
			}else{
				this.showSuccessJson("邮件发送失败");
			}
		}
		return JSON_MESSAGE;
	}
