...
@Results({
	 ...
	 @Result(name="self_list",type="freemarker", location="/b2b2c/admin/self/order/self_order_list.html"), //自营订单列表
	 ...
	 @Result(name="specifics",type="freemarker", location="/b2b2c/admin/order/order_specifics.html")
})
@Action("storeOrder")
...
public class StoreOrderAction extends WWAction{
	...
	public String list(){
		...
		if(orderTypeMap==null){
			...
			orderTypeMap = OrderType.getOrderTypeMapText();
			String p= JSONArray.fromObject(orderTypeMap).toString();
			orderType_Json=p.replace("[", "").replace("]", "");
		}
	...
	/** @description 新使用的admin订单详情页面 
	 * @date 2016年10月18日 上午9:55:11
	 * @return
	 * @return String
	 */
	public String specifics(){
		try{
			...
			if(!StringUtil.isEmpty(sn)){//订单
				order = storeOrderManager.get(orderId);
				bonus = bonusManager.getOrderBonus(null, orderId);
				JSONObject insureInfo = storeOrderManager.getInsureOrderDetail(orderId+"");
				JSONObject repairInfo = storeOrderManager.getRepairOrderDetail(orderId+"");
				orderMap.put("order", order); 
				orderMap.put("bonus", bonus);
				orderMap.put("insureOrderDetail", insureInfo);
				orderMap.put("repairOrderDetail", repairInfo);
			}else if(orderId!=null || orderId!=0){
				order =	storeOrderManager.get(sn);
				bonus = bonusManager.getOrderBonus(sn,null);
				orderMap.put("order", order);
				orderMap.put("bonus", bonus);
			}else{
			...
			orderMap.put("orderItemList", itemList); //订单货物列表
			orderMap.put("orderStatusText",  OrderStatus.getOrderStatusText(order.getStatus(), order.getOrder_type()));
		}catch(Exception e){
			e.printStackTrace();
		}
		return "specifics";
	 ...
	 * @return
	 */
	public String listSelfJson(){
		
		//定义店铺id为自营店的id
		this.store_id=ShopApp.self_storeid;
	...
	public String listJson(){
		...
		orderMap.put("ship_mobile", ship_mobile);
...
//		orderMap.put("shipping_type", shipping_type);
		...
		orderMap.put("order_type", order_type);
	...
	private Map getStatusJson(){
		Map orderStatus = new TreeMap();
		
		orderStatus.put(""+OrderStatus.ORDER_NOT_PAY, OrderStatus.getOrderStatusText(OrderStatus.ORDER_NOT_PAY));				// 未付款
		orderStatus.put(""+OrderStatus.ORDER_NOT_CONFIRM, OrderStatus.getOrderStatusText(OrderStatus.ORDER_NOT_CONFIRM));		// 议价中
		orderStatus.put(""+OrderStatus.ORDER_PAY_CONFIRM, OrderStatus.getOrderStatusText(OrderStatus.ORDER_PAY_CONFIRM));		// 已确认支付
//		orderStatus.put(""+OrderStatus.ORDER_ALLOCATION_YES, OrderStatus.getOrderStatusText(OrderStatus.ORDER_ALLOCATION_YES));	//备货完成
		orderStatus.put(""+OrderStatus.ORDER_SERVECE, OrderStatus.getOrderStatusText(OrderStatus.ORDER_SERVECE));				// 服务中
		orderStatus.put(""+OrderStatus.ORDER_APPRAISE, OrderStatus.getOrderStatusText(OrderStatus.ORDER_APPRAISE));				// 服务完成待评价
...
//		pmap.put(""+OrderStatus.PAY_PARTIAL_PAYED, OrderStatus.getPayStatusText(OrderStatus.PAY_PARTIAL_PAYED));
	...
	public String getOrderType_Json() {
		return orderType_Json;
	}
	public void setOrderType_Json(String orderType_Json) {
		this.orderType_Json = orderType_Json;
	}
	public Map getOrderTypeMap() {
		return orderTypeMap;
	}
	public void setOrderTypeMap(Map orderTypeMap) {
