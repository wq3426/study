...
public class SystemSetting {
	...
	private static String file_path;//上传文件的保存路径
	
	
	public static long car_position_push_interval=20;//车辆位置信息推送时间间隔
	
	public static final String setting_key="system"; //系统设置中的分组
	
	public static final String carmodel_image_savedir = "carmodelimage";//车型图片保存目录
	public static final String insure_company_image_savedir = "insuranceimage";//保险公司图片保存目录
	...
	public static void load(){
		...
		String car_position_push_interval_str = settings.get("car_position_push_interval");
		car_position_push_interval = StringUtil.isNull(car_position_push_interval_str) ? car_position_push_interval : Long.valueOf(car_position_push_interval_str);
	...
	public static String getFile_path() throws Exception{
		InputStream in = null;
		try {
			String os = System.getProperty("os.name");
			if(os.toLowerCase().contains("win")){//windows系统
				file_path = "C:/files/";
			}else if(os.toLowerCase().contains("linux")){//linux系统
			...
			props.load(in);
			String folder = props.getProperty("jdbc.filepath");
			file_path += folder;
