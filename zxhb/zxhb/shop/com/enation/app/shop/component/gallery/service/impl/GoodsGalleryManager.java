...
public class GoodsGalleryManager extends BaseSupport<GoodsGallery> implements IGoodsGalleryManager {
	...
	public List<GoodsGallery> list(int goods_id) {
		List<GoodsGallery> result = this.daoSupport.queryForList("select gg.*,g.params from es_goods_gallery gg left join es_goods g on gg.goods_id=g.goods_id where gg.goods_id = ?", GoodsGallery.class, goods_id);
		for (GoodsGallery gallery : result) {
...
//				String static_server_path= SystemSetting.getStatic_server_path();
				...
				String serverPath = file_path.replaceAll("\\\\", "/");
				filepath = filepath.replaceAll(static_server_domain+"/files", serverPath);
				filepath = filepath.replaceAll("\\\\","/");
				thumbName = thumbName.replaceAll(static_server_domain+"/files", serverPath);
