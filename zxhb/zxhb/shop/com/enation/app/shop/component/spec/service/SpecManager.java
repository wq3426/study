...
public class SpecManager extends BaseSupport<Specification>  implements ISpecManager {
	...
	public List<Specification> listSpecAndValueByType(int goodsTypeId){
		...
		String sql ="select s.* from specification s inner join type_spec ts on s.spec_id=ts.spec_id where ts.type_id=?";
		List<Specification> specList= this.baseDaoSupport.queryForList(sql,Specification.class, goodsTypeId);

		sql ="select * from spec_values where spec_id in (select spec_id from es_type_spec where type_id=?) order by spec_id";
		List valueList=   this.baseDaoSupport.queryForList(sql, new SpecValueMapper(), goodsTypeId);
		for(Specification spec :specList){
			List<SpecValue> newList =  new ArrayList<SpecValue>();
			for(SpecValue value:(List<SpecValue>)valueList){
				if(value.getSpec_id().intValue() == spec.getSpec_id().intValue()){
