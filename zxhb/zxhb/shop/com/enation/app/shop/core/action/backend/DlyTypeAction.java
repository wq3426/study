...
@Results({
	...
	@Result(name="exp", type="freemarker", location="/shop/admin/setting/check_exp.html")
})
public class DlyTypeAction extends WWAction {
	...
	public String saveAdd() {
		...
		try {
			...
			this.showErrorJson("配送方式添加失败"+e.getMessage());
	...
	public String delete() {
		try {
			...
			if (type_id.length >= total) {
				this.showErrorJson("不可全部删除，至少保留一条作为默认配送方式！");
			} else {
