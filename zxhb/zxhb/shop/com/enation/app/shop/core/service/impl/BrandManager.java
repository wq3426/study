...
//		return this.daoSupport.queryForList(sql);
	...
	public List<Brand> list() {
		String sql = "select * from brand where disabled=0 and type=0";
	...
	public List<Brand> goodsBrand() {
		String sql = "select * from brand where disabled=0 and type=1";
		List list = this.baseDaoSupport.queryForList(sql,new BrandMapper());
		return list;
	}
	...
	public List<Brand> listBrands(Integer tag_id){
		...
		List<Brand> brands = this.baseDaoSupport.queryForList(sql,Brand.class, tag_id);
		return brands; 
	}
	public void add(Brand brand) {	
		try {
			if(brand.getFile()!=null && brand.getFileFileName()!=null){
				brand.setLogo( UploadUtil.uploadFile(brand.getFile(), brand.getFileFileName(), "brand") );
			}
	...
	public List<Brand> listByCatId(Integer catid) {
		String sql ="select b.* from "+this.getTableName("brand")+" b ,"+this.getTableName("type_brand")+" tb,"+this.getTableName("goods_cat")+" c where tb.brand_id=b.brand_id and c.type_id=tb.type_id and c.cat_id=? order by b.ordernum desc";
	...
	public Page searchBrand(Map brandMap, int page, int pageSize) {
		...
		int type = (int) brandMap.get("type");
		
		String sql = "select * from brand where 1=1 and disabled=0 and type="+type+"";
	...
	@Override
	public Brand searchBrandNameIsExit(String brandname) {
		String sql = "select * from brand where name=?";
		Brand brand = this.baseDaoSupport.queryForObject(sql, Brand.class,
				brandname);// .queryForMap(sql, brand_id);
		return brand;
	}

	@Override
	public List getBrandByCatId(int cat_id) {
		String sql="SELECT eb.* FROM es_type_brand etb,es_brand eb,es_goods_cat egc WHERE  egc.type_id= etb.type_id  AND  etb.brand_id=eb.brand_id AND egc.cat_id = ?";
		List brandList =baseDaoSupport.queryForList(sql, Brand.class,cat_id);
		return brandList;
	}
