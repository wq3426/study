...
public class DlyTypeManager extends BaseSupport<DlyType> implements
	...
	private Page pageSelf(int pageNo,int pageSize){
		...
		Page page  = daoSupport.queryForPage(sql,pageNo,pageSize,ShopApp.self_storeid);
	...
	public Integer add(DlyType type, DlyTypeConfig config) {
		...
		int typeId= this.baseDaoSupport.getLastId("dly_type");
	...
	private void selfStoreSetting(int typeId){
		try {
			...
			Integer templateid = this.daoSupport.queryForInt(sql, ShopApp.self_storeid);
	...
	public int getDlyTotal(){
		String sql = "select count(0) from es_dly_type";
