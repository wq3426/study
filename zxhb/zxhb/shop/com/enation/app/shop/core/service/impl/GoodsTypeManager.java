...
public class GoodsTypeManager extends BaseSupport<GoodsType> implements IGoodsTypeManager {
	 ...
	 * @return
	 */
	public List getSpecListByTypeId(int type_id) {
		String sql ="select s.spec_name spec_name ,s.spec_id spec_id, 0 as num from "+this.getTableName("type_spec")+" ts inner join "+this.getTableName("specification")+" s  on s.spec_id = ts.spec_id where ts.type_id=?";
	...
	public Integer saveTypeSpec(GoodsType type){
		...
		if (spec_ids != null && spec_ids.length > 0) {
			for (Integer spec_id : spec_ids) {
				sql = "insert into  "+this.getTableName("type_spec")+"(type_id,spec_id) values(?,?)";
				simpleJdbcTemplate.update(sql, type_id, spec_id);
			}
		}
		return type_id;
	}
