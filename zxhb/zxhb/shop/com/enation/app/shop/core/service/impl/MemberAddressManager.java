...
public class MemberAddressManager extends BaseSupport<MemberAddress> implements
	...
	public void deleteAddress(int addr_id) {
		this.baseDaoSupport.execute(
				"update member_address set isDel = 1 where addr_id = ?", addr_id);
	}
	...
	public List<MemberAddress> listAddress() {
		...
		List<MemberAddress> list = this.baseDaoSupport.queryForList(
				"select * from member_address where member_id = ? and isDel = 0 order by def_addr desc", MemberAddress.class,  member.getMember_id());
		return list;
	}
	...
	public int addressCount(int member_id) {
		return baseDaoSupport.queryForInt("select count(*) from member_address where member_id=? and isDel = 0", member_id);
	}
	...
	@Override
	public List<MemberAddress> getMemberAddressList(Integer member_id) {
		StringBuffer sql = new StringBuffer();
		sql.append("select *from es_member_address where member_id = ?");
		return daoSupport.queryForList(sql.toString(), MemberAddress.class,member_id);
