...
public class MemberPointManger extends BaseSupport implements IMemberPointManger {
	...
	public void thaw(Integer orderId){
		...
		if(order.getStatus() == null || ( order.getStatus().intValue() != OrderStatus.ORDER_SERVECE/*ORDER_ROG*/  &&   order.getStatus().intValue() !=  OrderStatus.ORDER_COMPLETE ) ){
		...
		if(UserConext.getCurrentAdminUser()==null){
			...
			orderLog.setMessage("用户["+member.getUsername()+"]解冻订单["+orderId+"]积分，并将订单置为完成状态");
		}else{
		...
		for(FreezePoint fp:list){
			...
			//手动解冻收货订单的冻结积分
			if( fp.getOrder_status() == OrderStatus.ORDER_SERVECE/*ORDER_ROG*/  || order.getStatus().intValue() ==  OrderStatus.ORDER_COMPLETE ) {
				this.thaw(fp,true);
			}
		}
