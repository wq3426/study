...
public class OrderFlowManager extends BaseSupport implements IOrderFlowManager {
	...
	public void allocation(Allocation allocation ){
		...
		//更新订单的发货点，及订单状态
		this.baseDaoSupport.execute("update order set depotid=?,status=?,ship_status=?,allocation_time=? where order_id=?", allocation.getShipDepotId() ,OrderStatus.ORDER_SHIP,OrderStatus.SHIP_ALLOCATION_YES,DateUtil.getDateline(),allocation.getOrderid());
...
/*	public void shipping(Delivery delivery, List<DeliveryItem> itemList,List<DeliveryItem> giftItemList) {
	...
	*/
	...
	public void shipping(Delivery delivery, List<DeliveryItem> itemList) {
		...
		if(logger.isDebugEnabled()){
			logger.debug("更新订单["+ orderId+"]状态["+OrderStatus.ORDER_SERVECE/*ORDER_SHIP*/+"]，发货状态["+shipStatus+"]");
		}			
		//更新订单状态为已发货
		//2014-8-17 @author LiFenLong 增加增加发货时间
		String sql = "update order set status=?,ship_status=?,sale_cmpl_time=?";
		
		sql += " where order_id=?";
		baseDaoSupport.execute(sql, OrderStatus.ORDER_SERVECE/*ORDER_SHIP*/,shipStatus,DateUtil.getDateline(),orderId);
...
//		this.updateDepotStore(orderId);
	 ...
	 * 订单确认
	 */
	public void confirmOrder(Integer orderId , String backstage ) {
		if(orderId== null ) throw new  IllegalArgumentException("param orderId is NULL");
		Order order = orderManager.get(orderId);
		Member member = this.memberManager.get(order.getMember_id());
		
		if(order.getOrder_type() != OrderType.ZA_INSURANCE && order.getOrder_type() != OrderType.STORE_INSURANCE && StringUtil.isNull(backstage)){//保险订单，设置为议价状态
			this.daoSupport.execute("update es_order set status=?  where order_id=?", OrderStatus.ORDER_NOT_PAY ,orderId);
		}else if(!StringUtil.isNull(backstage) && backstage.equals("1")){//如果后台操作确认订单则全部都修改为未支付
			this.daoSupport.execute("update es_order set status=?  where order_id=?", OrderStatus.ORDER_NOT_PAY ,orderId);
		}
		//添加一条应收记录 20131110新增
...
//		}
		...
		String sql = "update " + this.getTableName("order") + " set status=" + orderStatus + ",pay_status=" + payStatus
				+ "  where order_id=?";
	...
	public void rogConfirm(int orderId,Integer op_id,String op_name,String sign_name,Long sign_time){
		...
		int orderstatus =OrderStatus.ORDER_APPRAISE/*ORDER_ROG*/; //默认是收货状态
	...
	public void cancel(String sn,String reason){
		try{
			...
			if(member==null){
				...
				this.log(order.getOrder_id(), "取消订单", member.getMember_id(), member.getUsername());
			}
