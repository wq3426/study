...
public class ProductManager extends BaseSupport<Product> implements IProductManager {
	...
	public void add(List<Product> productList) {
		 if(productList.size()>0){
			 ...
			 //清除规格信息
			 this.baseDaoSupport.execute("delete from  es_goods_spec  where goods_id=?",goodsid);
			 this.baseDaoSupport.execute("delete from  es_goods_lv_price  where goodsid=?", goodsid);
		 ...
		 for(Product product:productList){
			 ...
			 if( product_id==null ){
				 ...
				 product_id= this.baseDaoSupport.getLastId("es_product");
				 product.setProduct_id(product_id);
			 }else{
				 this.baseDaoSupport.update("es_product", product, "product_id="+ product_id);
			 }
			 ...
			 if(lvPriceList!=null){
				 for(GoodsLvPrice lvPrice  : lvPriceList){
					 ...
					 this.baseDaoSupport.insert("es_goods_lv_price", lvPrice);
				 }
		 ...
		 if(productList.size()>0){
			 Integer goodsid  =  productList.get(0).getGoods_id();
			 //更新商品的specs字段
			 this.baseDaoSupport.execute("update es_goods set specs=? where goods_id=?", JSONArray.fromObject(productList).toString(),goodsid);
		 } 
	}
...
//		sql="select sv.*,gs.product_id product_id from  "+ this.getTableName("goods_spec")+"  gs inner join "+this.getTableName("spec_values")+"  sv on gs.spec_value_id = sv.spec_value_id where  gs.goods_id=? order by gs.id desc" ;
		sql = "SELECT	sv.*, spe.spec_name,	spe.spec_memo,	gs.product_id product_id FROM	es_goods_spec gs,	es_spec_values sv,	es_specification spe WHERE 	gs.goods_id = ? AND gs.spec_value_id = sv.spec_value_id AND spe.spec_id = sv.spec_id ORDER BY gs.id DESC";
	...
	public void delete(Integer[] goodsid){
		...
		sql="delete from es_product_store where goodsid in("+id_str+")";
		this.baseDaoSupport.execute(sql);
	...
	@Override
	public Product getProductByCarplateAndType(String carPlate, String type, Object... args) {
		String sql = "";
		if(type.equals(OrderType.ZA_INSURANCE+"") || type.equals(OrderType.STORE_INSURANCE+"")){
			sql = "select goods_id from es_goods where name='汽车保险'";
		}
		if(type.equals(OrderType.REPAIR+"")){
			sql = "select goods_id from es_goods where name='汽车保养'";
		}
		int goods_id = daoSupport.queryForInt(sql);
		sql = "select product_id, name, goods_id from es_product where name=? and goods_id=?";
		List<Product> list = daoSupport.queryForList(sql, Product.class, carPlate, goods_id);
		return list.get(0);
	}
	...
	public void update(Product product) {
		String sql = "update es_product set price=?, enable_store=?, weight=0.0 where product_id=?";
		daoSupport.execute(sql, product.getPrice(), product.getEnable_store(), product.getProduct_id());
	}
