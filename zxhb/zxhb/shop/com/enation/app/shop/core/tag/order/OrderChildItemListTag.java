...
public class OrderChildItemListTag extends BaseFreeMarkerTag {
	...
	public Object exec(Map params) throws TemplateModelException {
		...
		for(Map oItem : itemListNew){
			int itemId = Integer.parseInt(oItem.get("item_id").toString());
			int isPack =0;
			if(oItem.get("is_pack")!=null){
				isPack = Integer.parseInt(oItem.get("is_pack").toString());	
			}
			
			if(isPack == 1){
				List child = orderManager.getOrderItemDetail(itemId);
				list.addAll(child);
			}
			
		}
		map.put("childItemList", list);
		return map;
	}
	/**
